const HDQ = {
	EL: {
		upload: document.getElementById("bitc_csv_file_upload"),
		begin: document.getElementById("bitc_start_csv_upload"),
	...
	init: function () {
		...
		if (HDQ.EL.upload != null) {
			HDQ.EL.upload.addEventListener("change", function (e) {
				...
				jQuery(".bitc_file_label").html(filename);
			});
		}
	},
	uploadCSV: function () {
	...
	parseNext: function (path, total = null) {
		...
		jQuery.ajax({
			...
			success: function (data) {
				...
				if (HDQ.VARS.current != HDQ.VARS.total) {
					let item = `<div class = "bitc_log_item">added ${HDQ.VARS.current} / ${HDQ.VARS.total} questions</div>`;
					HDQ.EL.log.insertAdjacentHTML("afterbegin", item);
					setTimeout(function () {
						HDQ.parseNext(p);
					}, 2000); // delay to stop overloading slow servers
				} else {
					let item = `<div class = "bitc_log_item" style = "color:darkseagreen">ALL QUESTIONS HAVE BEEN ADDED<br/><br/>added ${HDQ.VARS.total} / ${HDQ.VARS.total} questions</div>`;
					HDQ.EL.log.insertAdjacentHTML("afterbegin", item);
				}
			},
			error: function () {
				let item = `<div class = "bitc_log_item" style = "color:darkred">THERE WAS A SERVER ERROR ADDING ONE OF YOUR QUIZZES</div>`;
				HDQ.EL.log.insertAdjacentHTML("afterbegin", item);
			},
		});
	},
